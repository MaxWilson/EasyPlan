...
type [<AllowNullLiteral>] FileContainerRestClient =
    ...
    abstract createItems: items: WebApi.VssJsonCollectionWrapperV<ResizeArray<FileContainer.FileContainerItem>> * containerId: int * ?scope: string -> Promise<ResizeArray<FileContainer.FileContainerItem>>
    /// <summary>Deletes the specified items in a container.</summary>
    /// <param name="containerId">- Container Id.</param>
    /// <param name="itemPath">- Path to delete.</param>
    /// <param name="scope">- A guid representing the scope of the container. This is often the project id.</param>
    abstract deleteItem: containerId: int * itemPath: string * ?scope: string -> Promise<unit>
    ...
    abstract getItems: containerId: int * ?scope: string * ?itemPath: string * ?metadata: bool * ?format: string * ?downloadFileName: string * ?includeDownloadTickets: bool * ?isShallow: bool -> Promise<ResizeArray<FileContainer.FileContainerItem>>
