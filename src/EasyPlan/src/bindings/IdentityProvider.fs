...
type [<AllowNullLiteral>] IPeoplePickerProvider =
    ...
    abstract getEntityFromUniqueAttribute: (string -> U2<IIdentity, Promise<IIdentity>>) with get, set
    /// If no input is in the search box when clicked, provide a set of identities to show (used for MRU)
    abstract onEmptyInputFocus: (unit -> U2<ResizeArray<IIdentity>, Promise<ResizeArray<IIdentity>>> option) option with get, set
    /// Given a list of currently selected items and a filter string, return a list of suggestions to put in the suggestion list
    abstract onFilterIdentities: (string -> ResizeArray<IIdentity> -> U2<ResizeArray<IIdentity>, Promise<ResizeArray<IIdentity>>> option) with get, set
    /// Request connection information about a given Entity.
    abstract onRequestConnectionInformation: (IIdentity -> bool -> U2<IdentitiesGetConnectionsResponseModel, Promise<IdentitiesGetConnectionsResponseModel>>) with get, set
...
type [<AllowNullLiteral>] PeoplePickerProvider =
    ...
    abstract getEntityFromUniqueAttribute: (string -> U2<IIdentity, Promise<IIdentity>>) with get, set
    /// If no input is in the search box when clicked, provide a set of identities to show (used for MRU)
    abstract onEmptyInputFocus: (unit -> U2<ResizeArray<IIdentity>, Promise<ResizeArray<IIdentity>>>) with get, set
    /// Given a list of currently selected items and a filter string, return a list of suggestions to put in the suggestion list
